#!/usr/bin/env python3
"""
テスト用ログファイル生成スクリプト（ソーシャルゲーム向け）

JSONL形式のgzip圧縮ログを生成します。
"""
...
def save_logs_to_jsonl_gz(logs: list, output_path: Path):
    """ログをJSONL形式でgzip圧縮して保存"""
...
def main():
    """メイン処理"""
    # 出力ディレクトリ
    output_dir = Path(__file__).parent.parent / "test-data" / "logs"
        ...
        for days_ago in range(7):
            ...
            for hour in range(24):
                ...
                output_path = (output_dir / f"team_id={team_id}" /
                              ...
                              f"hour={hour:02d}" /
                              f"logs-{hour:02d}.jsonl.gz")
                save_logs_to_jsonl_gz(logs, output_path)
                total_files += 1
    ...
    print("\nNext steps:")
    print("1. Get S3 bucket name from amplify_outputs.json")
    print("2. Upload logs using: ./scripts/upload-test-logs.sh")
